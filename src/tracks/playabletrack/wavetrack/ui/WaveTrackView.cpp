...
void WaveTrackView::Copy( const TrackView &other )
{
   TrackView::Copy( other );

   if ( const auto pOther = dynamic_cast< const WaveTrackView* >( &other ) ) {
   }
}
...
static WaveTrack::Caches::RegisteredFactory sKey2{ []( WaveTrack& ){
...
WaveTrackLocationsCache &WaveTrackLocationsCache::Get( const WaveTrack &track )
{
   return const_cast< WaveTrack& >( track )
      .Caches::Get< WaveTrackLocationsCache >( sKey2 );
}
