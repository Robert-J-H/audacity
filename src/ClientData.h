...
/*
   ...
   See also alias templates Extensions and Caches that can abbreviate the
   template parameters supplied to Site in common cases.
   ...
   class CoreObject // The object hosting the attachments
      : public ClientData::Site< CoreObject, AbstractClientData >
   ...
   {
   public:
      MyClientData( CoreObject &host )
      {
         // ... use host, maybe keep a back pointer to it, maybe not,
         // depending how CoreObject declares NotificationMethod ...
         // ... Maybe CoreObject too is an abstract class and we invoke some
   ...
   // when a CoreObject uses BuildAll, or else lazily when client code uses
   // CoreObject::Get()
   static const CoreObject::RegisteredFactory key{
      []( CoreObject &host ){ return std::make_unique< MyClientData >( host ); }
   };

   // Use of that key at other times, not dependent on notifications from
   // the core
   void DoSomething( CoreObject &host )
   {
   ...
   void DoAnotherThing( CoreObject &host )
   {
   ...
   void DoYetAnotherThing( CoreObject &host )
   {
...
private:
   ...
   Slot( Locked<DataContainer> &data, const RegisteredFactory &key, bool create )
   {
      ...
      EnsureIndex( data, index );
      auto iter = GetIterator( data, index );
      auto &pointer = create ? Ensure( data, iter, index ) : *iter;
   ...
   DataPointer &Ensure( Locked<DataContainer> &,
